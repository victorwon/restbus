...
routes.fetchIntoCache = function(agencyId, routeId, callback) {
  var path = [NBXML_FEED, '?command=routeConfig&a=', agencyId, '&r=', routeId].join('');

  if (nbcache[agencyId] && nbcache[agencyId].routes[routeId]) {
    console.log("++++++++ return cache for :" + agencyId + " -> " + routeId);
    callback();
    return;
  }
      ...
      if (!err) {
        ...
        if(!nberr) {
          ...
          route.stop.forEach(function(stop) {
            ...
            json.stops[$.tag] = { id: $.tag, code: code, title: $.title, lat: lat, lon: lon };
          });
          ...
          if (!nbcache[agencyId]) {
            nbcache[agencyId] = {};
            nbcache[agencyId].routes = [];
          }
          nbcache[agencyId].routes[routeId] = json;
          callback();
          return;
          
        } else {
  		  console.log(" ****** NextBus XML Failed ****** ");
		  callback(nberr);
        }
      } else {
