          ...
          if(!parseErr) {
            ...
            if(nbjson.preds) {
              nbjson.preds.forEach(function(pred) {
                ...
                routeSet.add(pfs.a+'|||'+pfs.r); // collecting routes
            	...
            	// -- Fetch all involved routes data into cache, then use it to fulfill the lat,lon for each stop
				async.eachLimit(Array.from(routeSet), 4, function(item, callbackDone) {
					console.log("- "+item);
					var t = item.split('|||');
					routes.fetchIntoCache(t[0], t[1], callbackDone);
					...
					// saturate the lat.lon to each stop
					json.forEach(function(pr) {
						var stop = nbcache[pr.agency.id].routes[pr.route.id].stops[pr.stop.id];
						pr.stop.lat = stop.lat;
						pr.stop.lon = stop.lon;
					});
					res.json(200, json);
